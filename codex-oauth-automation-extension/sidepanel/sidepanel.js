...
const inputVerificationCodeRetryEnabled = document.getElementById('input-verification-code-retry-enabled');
const inputVerificationCodeRetryCount = document.getElementById('input-verification-code-retry-count');
...
function updateVerificationCodeRetryInputState() {
  if (!inputVerificationCodeRetryCount) {
    return;
  }

  inputVerificationCodeRetryCount.disabled = !inputVerificationCodeRetryEnabled.checked;
}
...
function collectSettingsPayload() {
  ...
  return {
    ...
    verificationCodeRetryEnabled: inputVerificationCodeRetryEnabled.checked,
    verificationCodeRetryCount: normalizeVerificationCodeRetryCount(inputVerificationCodeRetryCount.value),
...
function applySettingsState(state) {
  ...
  inputVerificationCodeRetryEnabled.checked = Boolean(state?.verificationCodeRetryEnabled);
  inputVerificationCodeRetryCount.value = String(normalizeVerificationCodeRetryCount(state?.verificationCodeRetryCount));
  ...
  if (state?.autoRunTotalRuns !== undefined) {
  ...
  updateAutoDelayInputState();
  updateFallbackThreadIntervalInputState();
  updateVerificationCodeRetryInputState();
  updatePanelModeUI();
  updateMailProviderUI();
  updateButtonStates();
}
...
function updateMailProviderUI() {
  ...
  const fetchButtonLabel = useGeneratedAlias ? '生成' : uiCopy.buttonLabel;
  ...
  btnFetchEmail.textContent = fetchButtonLabel;
  if (autoHintText) {
...
inputVerificationCodeRetryEnabled.addEventListener('change', () => {
  updateVerificationCodeRetryInputState();
  markSettingsDirty(true);
  saveSettings({ silent: true }).catch(() => { });
});

inputVerificationCodeRetryCount.addEventListener('input', () => {
  markSettingsDirty(true);
  scheduleSettingsAutoSave();
});
inputVerificationCodeRetryCount.addEventListener('blur', () => {
  inputVerificationCodeRetryCount.value = String(
    normalizeVerificationCodeRetryCount(inputVerificationCodeRetryCount.value)
  );
  saveSettings({ silent: true }).catch(() => { });
});
...
chrome.runtime.onMessage.addListener((message) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.verificationCodeRetryEnabled !== undefined) {
        inputVerificationCodeRetryEnabled.checked = Boolean(message.payload.verificationCodeRetryEnabled);
        updateVerificationCodeRetryInputState();
      }
      if (message.payload.verificationCodeRetryCount !== undefined) {
        inputVerificationCodeRetryCount.value = String(
          normalizeVerificationCodeRetryCount(message.payload.verificationCodeRetryCount)
        );
      }
